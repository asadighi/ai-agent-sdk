...
export const AgentGraph = ({ width = 800, height = 600, heartbeats, logger }) => {
    ...
    return (<Box sx={{ width, height, border: '1px solid #ccc', borderRadius: 1, position: 'relative' }}>
      <canvas ref={canvasRef} width={width} height={height} onMouseMove={handleMouseMove} onClick={handleClick} style={{ width: '100%', height: '100%' }}/>
      <NodeInfoPanel node={selectedNode} isOpen={isPanelOpen} onClose={handleClosePanel}/>
    </Box>);
};
AgentGraph.propTypes = {
    width: PropTypes.number,
    height: PropTypes.number,
    heartbeats: (props, propName, componentName) => {
        const prop = props[propName];
        if (!(prop instanceof Map)) {
            return new Error(`Invalid prop \`${propName}\` supplied to \`${componentName}\`. Expected a Map.`);
        }
        for (const [key, value] of prop.entries()) {
            if (typeof key !== 'string') {
                return new Error(`Invalid prop \`${propName}\` supplied to \`${componentName}\`. Map keys must be strings.`);
            }
            if (!value || typeof value !== 'object') {
                return new Error(`Invalid prop \`${propName}\` supplied to \`${componentName}\`. Map values must be objects.`);
            }
            if (!('agentId' in value) || !('role' in value) || !('status' in value) || !('timestamp' in value)) {
                return new Error(`Invalid prop \`${propName}\` supplied to \`${componentName}\`. Map values must have required Heartbeat properties.`);
            }
        }
        return null;
    },
