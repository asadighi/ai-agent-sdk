...
import { AgentRole, AgentStatus, MeshClient, Agent, Mesh, Logger, LogLevel, BrowserStorage } from '@ai-agent/core-sdk';
import { ThemeProvider, defaultTheme, BaseAgentGraph, StatusBadge } from '@ai-agent/web-sdk';
import './App.css';
import { Container, Typography, Box, Paper, TextField, Button, Select, MenuItem, FormControl, InputLabel, Alert, Tabs, Tab } from '@mui/material';
...
function App() {
    ...
    const [heartbeats, setHeartbeats] = useState(new Map());
    ...
    const [meshId, setMeshId] = useState(searchParams.get('meshId') || uuidv4());
    const [agentId, setAgentId] = useState(searchParams.get('agentId') || uuidv4());
    const [workerType, setWorkerType] = useState(searchParams.get('workerType') || AgentRole.Worker);
    ...
    const withRetry = useCallback(async (operation, maxRetries = 3, baseDelay = 5000, cacheKey) => {
        let lastError = null;
        if (isOffline) {
    ...
    }, [isOffline, isQuotaExceeded, rateLimit, lastRequestTime, logger, setStatusMessage, setIsQuotaExceeded, setRateLimit, setQuotaResetTime, localCache, lastCacheUpdate, setLocalCache, setLastCacheUpdate]);
    useEffect(() => {
        const initializeClient = async () => {
            try {
                ...
                connectionStateRef.current.subscribeToConnectionState((isOnline) => {
                ...
                setStatusMessage(`Failed to initialize client: ${e instanceof Error ? e.message : String(e)}`);
            }
            finally {
                setIsLoading(false);
            }
    ...
    }, [client, meshId, isOffline, heartbeats, logger, setStatusMessage, setHeartbeats]);
    ...
    const startAgent = async () => {
        ...
        if (isOffline) {
            setStatusMessage('Cannot start agent while offline');
            return;
        }
    ...
    const reconnectAgent = async () => {
        ...
        if (isOffline) {
            setStatusMessage('Cannot reconnect agent while offline');
            return;
        }
        setIsLoading(true);
    ...
    return (<ThemeProvider theme={defaultTheme}>
      <Container maxWidth="lg">
        <Box sx={{ my: 4 }}>
          {isOffline && (<Alert severity="error" sx={{
                mb: 2,
                position: 'sticky',
                top: 0,
                zIndex: 1000
          ...
          <Paper sx={{ p: 2, mb: 2, backgroundColor: '#f5f5f5' }}>
            ...
            {quotaResetTime && (<Typography variant="body2" color="error" sx={{ mt: 1 }}>
          ...
          {activeTab === 0 && (<>
              <Paper sx={{ p: 2, mb: 2 }}>
                <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
                  <FormControl sx={{ minWidth: 120 }}>
                    ...
                    <Select value={workerType} label="Role" onChange={(e) => {
                ...
                setSearchParams(params);
                  ...
                  <TextField label="Mesh ID" value={meshId} onChange={(e) => {
                ...
                setSearchParams(params);
                  ...
                  <TextField label="Agent ID" value={agentId} onChange={(e) => {
                ...
                setSearchParams(params);
                  ...
                  <Button variant="contained" onClick={handleAgentAction} disabled={isLoading || !!agent || !meshId || !agentId || !client}>
                    {isLoading
                ? (agentExists ? 'Reconnecting...' : 'Starting...')
                : (agentExists ? 'Reconnect Agent' : 'Start Agent')}
                  </Button>
                  <Button variant="outlined" onClick={stopAgent} disabled={!agent || isLoading}>
                    Stop Agent
                  </Button>
                </Box>

                {agent && (<Paper sx={{ p: 2, mt: 2, bgcolor: '#f5f5f5' }}>
              ...
              <Paper sx={{ p: 2, mb: 2, minHeight: '400px' }}>
                <Typography variant="h6" gutterBottom>Agent Network</Typography>
                <BaseAgentGraph agents={heartbeats}/>
              </Paper>
            </>)}

          {activeTab === 1 && (<Paper sx={{ p: 2, mb: 2, minHeight: '400px' }}>
              <Typography variant="h6" gutterBottom>Logs</Typography>
              <LogViewer logs={heartbeats}/>
            </Paper>)}
        </Box>
      </Container>
    </ThemeProvider>);
